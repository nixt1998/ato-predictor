...
import { Download, FileText, Loader2, CheckCircle2, XCircle, X } from 'lucide-react'
import { SavedPrediction } from '@/types/history'

interface DownloadPDFDialogProps {
  isOpen: boolean
  record: SavedPrediction | null
  onClose: () => void
}
 ...
 * PDF下载对话框 - 用于历史记录单条下载
 */
export default function DownloadPDFDialog({ isOpen, record, onClose }: DownloadPDFDialogProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/generate-report', {
        ...
        body: JSON.stringify({ language, predictionData }),
      })
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-white/80">{record.nickname || record.reportNumber}</p>
              </div>
            </div>
            <button
              onClick={onClose}
